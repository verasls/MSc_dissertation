...
As already discussed, the accelerometers output can be either activity counts, a dimensionless unit, or raw acceleration, usually expressed as gravitational acceleration units (\textit{g}). Both outputs can indicate overall movement, but they need to be converted into more biologically meaningful units in a process designated as calibration \cite{Welk_2005}. This process can be one of two types: value calibration, which examines the accelerometers validity, and unit calibration, which analyses its reliability by measuring differences in the output among distinct units of the same device \cite{Welk_2005}. The remaining of this section will address value calibration.
